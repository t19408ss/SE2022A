...
DATA *
alloc_data(int key, int val, DATA *next)
{
	DATA *data;
...
// DATA *
// scan_data(NODE *node, int key_begin, int range)
// {
// 	int kid, key, r;
// 	DATA *head;
// 	NODE *leaf;
...
DATA *
find_data_without_index(DATA *head, int key)
{
	DATA *target = head;
	do {
		if (target->key == key) {
...
std::atomic<int> num_commit;
std::atomic<int> num_abort;
